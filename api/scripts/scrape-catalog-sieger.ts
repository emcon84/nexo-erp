/**
 * Scrape del catálogo de la marca SIEGER desde la API JSON pública.
 *
 ...
 * ej "3 kg, 15 kg") y una imagen `url_frente` (una foto GENÉRICA por receta/linea).
 *
 * Salida: `scripts/data/catalog-sieger.json` — array de CatalogItem normalizado.
 *
 * CLI flags:
 *   --dry        (default) solo escribe el JSON.
 *
 * Uso (cwd = api/):
 *   npx ts-node scripts/scrape-catalog-sieger.ts
 */
...
const OUT_FILE = path.join(OUT_DIR, "catalog-sieger.json");
...
async function fetchPage(page: number): Promise<{ items: ApiItem[]; total: number } | null> {
  try {
    ...
    const json = (await res.json()) as { status?: boolean; items?: ApiItem[]; total?: number };
...
async function main() {
  ...
  console.log(`🔍 Scrapeando catálogo SIEGER via ${API_URL}`);
